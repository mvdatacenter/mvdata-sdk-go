...
copy-attributevalue-feature:
	...
	ls *.go | grep -v "convert.go" | grep -v "doc.go" | \
		xargs -I % sed -i.bk -E 's:github.com/mvdatacenter/mvdata-sdk-go/(service|feature)/dynamodb:github.com/mvdatacenter/mvdata-sdk-go/\1/dynamodbstreams:g' % &&  \
	ls *.go | grep -v "convert.go" | grep -v "doc.go" | \
...
sdkv1check:
	@echo "Checking for usage of MVData SDK for Go v1"
...
list-deps-%:
	...
	@cd ./internal/repotools/cmd/eachmodule \
		...
		"go list -m all | grep -v 'github.com/mvdatacenter/mvdata-sdk-go'" | sort -u
